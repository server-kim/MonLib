...
namespace MonLib
{
    ...
    bool CLanClient::Connect(WCHAR *p_IP, USHORT Port, int WorkerThreadCnt, bool Nagle)
    {
    ...
    bool CLanClient::SendPacket(Packet *p_Packet)
    {
        //if (false == _Connect)
        //    return false;
    ...
    //bool CLanClient::GetClientConnect(void)
    //{
    //    if (1 == _p_IOCompare->ReleaseFlag)
    //        return false;
    //    else
    //        return true;
    //}
    int CLanClient::GetRecvQSize(void)
    {
        return _RecvQ.GetUseSize();
    }
    bool CLanClient::IsConnect(void)
    {
        if (1 == _p_IOCompare->ReleaseFlag)
            return false;
        return true;
    ...
    int CLanClient::CompletePacket(void)
    {
        Packet *p_Packet;
        int RecvQSize;
        st_LAN_HEADER Header;
        ...
        // payload size check
        _RecvQ.Peek((char *)&Header, sizeof(st_LAN_HEADER));
        ...
        if (Header.PayloadSize + sizeof(st_LAN_HEADER) > RecvQSize)
            return Packet_NotComplete;
        if (false == _RecvQ.RemoveData(sizeof(st_LAN_HEADER)))
            return Packet_Error;
        ...
        //p_Packet->SetHeader_CustomHeader((char *)&Header, sizeof(st_LAN_HEADER));
        p_Packet->SetHeader_CustomHeader_Short(Header.PayloadSize);
        if (Header.PayloadSize != _RecvQ.Get((char *)p_Packet->GetWriteBufferPtr(), Header.PayloadSize))
        {
            p_Packet->Free();
            return Packet_Error;
        }
    ...
    void CLanClient::CompleteRecv(DWORD cbTransferred)
    {
    ...
    void CLanClient::CompleteSend(DWORD cbTransferred)
    {
    ...
    void CLanClient::ClientShutdown(void)              // shutdown으로 안전한 종료 유도
    {
        shutdown(_Sock, SD_SEND);
    }
    void CLanClient::ClientDisconnect(void)            // socket 접속 강제 끊기
    {
        shutdown(_Sock, SD_BOTH);     // shutdown both
    }
    void CLanClient::SocketClose(void)                    // closesocket(내부용)
    {
        linger Ling;
        Ling.l_onoff = 1;
        Ling.l_linger = 0;
    ...
    void CLanClient::ClientRelease(void)               // client session 해제
    {
    ...
    unsigned __stdcall CLanClient::WorkerThreadProc(LPVOID lpParam)
    {
        CLanClient *p_This;
        BOOL Ret;
        DWORD cbTransferred;
