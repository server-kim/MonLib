...
namespace MonLib
{
    CNetServerEx4::CNetServerEx4(int SessionMax, bool Nagle)
    {
        if (SessionMax < 1)
        {
        ...
        _hIocpAccept = INVALID_HANDLE_VALUE;
        for (Cnt = 0; Cnt < ACCEPT_EX_THREAD_MAX_COUNT; ++Cnt)
        {
            _AcceptExThread[Cnt] = INVALID_HANDLE_VALUE;
            _AcceptExThreadId[Cnt] = 0;
        }
        ...
        _SocketPool = new SocketPoolEx(_SessionMax * 2);
    }
    CNetServerEx4::~CNetServerEx4(void)
    {
        stop();
        WSACleanup();
    }

    bool CNetServerEx4::start(WCHAR *p_IP, USHORT Port, int WorkerThreadCnt, BYTE PacketCode, BYTE XORCode1, BYTE XORCode2, bool SendThread)
    {
        if (true == _ServerOn)
            return false;
        _ServerOn = true;
    ...
    bool CNetServerEx4::NetworkInit(void)
    {
        //----------------------------------------------------
        // network init
        //----------------------------------------------------
        ...
        hResult = CreateIoCompletionPort((HANDLE)_ListenSock, _hIocpAccept, (ULONG_PTR)COMPLETION_KEY_OVERLAPPED, 0);
        if (NULL == hResult)
        {
    ...
    bool CNetServerEx4::AcceptExInit(void)
    {
        GUID GuidAcceptEx;
        DWORD dwBytes;
        int Ret;
    ...
    bool CNetServerEx4::DisconnectExInit(void)
    {
    ...
    bool CNetServerEx4::SetAcceptEx(SOCKET *ListenSock, st_NETWORK_SOCKET *p_NetworkSocket)
    {
        int Ret;
        BOOL bRet;
        DWORD dwBytes;
    ...
    void CNetServerEx4::stop(void)
    {
        ...
        for (Cnt = 0; Cnt < ACCEPT_EX_THREAD_MAX_COUNT; ++Cnt)
        {
            PostQueuedCompletionStatus(_hIocpAccept, 1, (ULONG_PTR)COMPLETION_KEY_EXIT, NULL);
        }
        // 스레드 중단 확인(이게 생각보다 시간이 오래걸림.)
        for (Cnt = 0; Cnt < ACCEPT_EX_THREAD_MAX_COUNT; ++Cnt)
        {
            if (WaitForSingleObject(_AcceptExThread[Cnt], 10000) != WAIT_OBJECT_0)
            {
                SYSLOG(L"SYSTEM", LOG_SYSTEM, L"Stop # [Error][Terminate Accept Thread]");
                TerminateThread(_AcceptExThread[Cnt], 0);
            }
        }
        // 스레드 핸들 반환
        for (Cnt = 0; Cnt < ACCEPT_EX_THREAD_MAX_COUNT; ++Cnt)
        {
            CloseHandle(_AcceptExThread[Cnt]);
            _AcceptExThread[Cnt] = INVALID_HANDLE_VALUE;
            _AcceptExThreadId[Cnt] = 0;
        }
        SYSLOG(L"SYSTEM", LOG_SYSTEM, L"Stop # [2][Accept Thread Exit]");
        CloseHandle(_hIocpAccept);
        _hIocpAccept = INVALID_HANDLE_VALUE;
        SYSLOG(L"SYSTEM", LOG_SYSTEM, L"Stop # [2-1][Iocp Released]");
    ...
    LONG64 CNetServerEx4::GetSessionCount(void)
    {
        return _SessionCount;
    }
    bool CNetServerEx4::SendPacketSendThread(__int64 SessionID, Packet *p_Packet, bool Disconnect)
    {
    ...
    bool CNetServerEx4::GetServerOn(void)
    {
        return _ServerOn;
    }
    __int64 CNetServerEx4::GetSocketUseCount(void)
    {
        return _SocketPool->GetUseCount();
    }
    __int64 CNetServerEx4::GetSocketAllocCount(void)
    {
        return _SocketPool->GetAllocCount();
    }
    int CNetServerEx4::GetEmptyStackUseSize(void)
    {
    ...
    bool CNetServerEx4::Disconnect(__int64 ClientID)
    {
    ...
    unsigned __int64 CNetServerEx4::FindEmptySession(void)
    {
        // lockfree stack ver.
    ...
    CNetServerEx4::st_SESSION *CNetServerEx4::FindSession(__int64 SessionID)
    {
    ...
    int CNetServerEx4::CompletePacket(st_SESSION *p_Session)
    {
        Packet *p_Packet;
        int RecvQSize;
    ...
    int CNetServerEx4::CompleteRecvPacket(st_SESSION *p_Session)
    {
        int RecvQSize;
        st_PACKET_HEADER Header;
        Packet *p_Packet;
    ...
    bool CNetServerEx4::RecvPost(st_SESSION *p_Session)
    {
        int BufCount;
        WSABUF RecvWsaBuf[2];
        DWORD flags;
    ...
    bool CNetServerEx4::SendPostIOCP(__int64 SessionID)
    {
        st_SESSION *p_Session;
    ...
    void CNetServerEx4::CompleteRecv(st_SESSION *p_Session, DWORD cbTransferred)
    {
        // 문제가 있는 상황. 죽어랏!!
    ...
    void CNetServerEx4::CompleteSend(st_SESSION *p_Session, DWORD cbTransferred)
    {
    ...
    void CNetServerEx4::ClientShutdown(st_SESSION *p_Session)              // shutdown으로 안전한 종료 유도
    {
        shutdown(p_Session->p_NetworkSocket->_Sock, SD_SEND);
    }
    void CNetServerEx4::ClientDisconnect(st_SESSION *p_Session)            // socket 접속 강제 끊기
    {
        shutdown(p_Session->p_NetworkSocket->_Sock, SD_BOTH);              // shutdown both
    }
    void CNetServerEx4::SocketClose(SOCKET SessionSock)                    // closesocket(내부용)
    {
        //linger Ling;
        //Ling.l_onoff = 1;
        //Ling.l_linger = 0;
    ...
    void CNetServerEx4::SocketReturn(st_NETWORK_SOCKET *p_NetworkSocket)
    {
        //p_NetworkSocket->Type = OVERLAPPED_TYPE_ACCEPT;
        //p_NetworkSocket->_p_Data = &_ListenSock;
    ...
    //void CNetServerEx4::ClientRelease(st_SESSION *p_Session)               // client session 해제
    //{
    ...
    //}
    void CNetServerEx4::ClientRelease(st_SESSION *p_Session)
    {
        st_IO_COMPARE ReleaseCompare;
        ReleaseCompare.IOCount = 0;
        ReleaseCompare.ReleaseFlag = 0;
    ...
    //void CNetServerEx4::ClientReleaseEx(st_SESSION *p_Session)               // client session 해제
    //{
    //    Packet *p_Packet;
    //    int ReleaseSessionIndex;
    //    
    ...
    //    _SessionEmptyStack->Push(ReleaseSessionIndex);
    //}
    void CNetServerEx4::SocketRelease(st_NETWORK_SOCKET *p_NetworkSocket)
    {
        p_NetworkSocket->Type = OVERLAPPED_TYPE_ACCEPT;
        p_NetworkSocket->_p_Data = &_ListenSock;
        if (false == _SocketPool->FreeSocket(p_NetworkSocket))
    ...
    unsigned __stdcall CNetServerEx4::MonitorTPS_Thread(LPVOID lpParam)
    {
        return ((CNetServerEx4 *)lpParam)->MonitorTPS_Thread_update();
    }
    unsigned CNetServerEx4::MonitorTPS_Thread_update(void)
    {
    ...
    unsigned __stdcall CNetServerEx4::AcceptExThreadProc(LPVOID lpParam)
    {
        return ((CNetServerEx4 *)lpParam)->AcceptExThread_Update();
    }
    unsigned CNetServerEx4::AcceptExThread_Update(void)
    {
        // GQCS
        DWORD cbTransferred;
        ...
        while (1)
        {
            ...
            GQCSRet = GetQueuedCompletionStatus(_hIocpAccept, &cbTransferred, (PULONG_PTR)&p_CompletionKey, (LPOVERLAPPED *)&lpOverlapped, INFINITE);
    ...
    unsigned __stdcall CNetServerEx4::WorkerExThreadProc(LPVOID lpParam)
    {
        return ((CNetServerEx4 *)lpParam)->WorkerExThread_Update();
    }
    unsigned CNetServerEx4::WorkerExThread_Update(void)
    {
        // GQCS
        DWORD cbTransferred;
        ULONG_PTR p_CompletionKey;
        ...
        while (1)
        {
            ...
            case COMPLETION_KEY_OVERLAPPED:
                ...
                case OVERLAPPED_TYPE_DISCONNECT:
                    ...
                    {
                        LastErrorCode = GetLastError();
                        SYSLOG(L"SYSTEM", LOG_SYSTEM, L"Disconnect # [GQCS Failed with Error: %u]", LastErrorCode);
                        //SocketRelease(p_NetworkSocket);
                        //SocketReturn(p_NetworkSocket);
                        //break;
                    }
    ...
    //    return 0;
    //}
    unsigned __stdcall CNetServerEx4::SendThreadProc(LPVOID lpParam)
    {
        return ((CNetServerEx4 *)lpParam)->SendThread_Update();
    }
    unsigned CNetServerEx4::SendThread_Update(void)
    {
        int SessionIndex;
        st_SESSION *p_Session;
        __int64 SessionID;
